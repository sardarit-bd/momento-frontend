...
const getCanonicalCardType = (value = "") => {
  ...
  if (normalized.includes("jeck") || normalized.includes("jack"))
    return "Jeck_Card";
...
const ProductCustomizer = () => {
    ...
    const fetchProduct = async () => {
      ...
      if (savedCards) {
        ...
        const sanitizedCards = parsedCards.map((card) => {
          ...
          const hasValidBase = validBaseByType[canonicalType]?.has(
            card?.baseImage,
          );
          const shared = {
          ...
          if (hasValidBase)
            return {
              ...
              slotName:
                card?.slotName || firstBaseNameByType[canonicalType] || null,
              ...shared,
            };
          return {
            ...card,
        ...
        sanitizedCards.sort(
          (a, b) =>
            flowOrder.indexOf(a.editedCard) - flowOrder.indexOf(b.editedCard),
        );

        if (sanitizedCards.length > 0) {
          localStorage.setItem(
            customCardsStorageKey,
        ...
        const initialType =
          CARD_FLOW.find((type) =>
            sanitizedCards?.some((card) => card?.editedCard === type),
          ) || sanitizedCards?.[0]?.editedCard;
        const storedActiveIndex = Number(
          localStorage.getItem(customCardsActiveIndexStorageKey),
        );
        const initialIndex =
          Number.isInteger(storedActiveIndex) &&
          storedActiveIndex >= 0 &&
          storedActiveIndex < sanitizedCards.length
            ? storedActiveIndex
            : sanitizedCards?.findIndex(
                (card) => card?.editedCard === initialType,
              );
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      customCardsActiveIndexStorageKey,
      String(activeCardIndex),
    );
  }, [
    cards,
    activeCardIndex,
  ...
  const selectBaseImage = (url, type, slotName = null) => {
    ...
    if (isDuplicate) {
      toast.warn(
        `${CARD_TYPE_LABELS[type] || "This card"} is already added. Remove it first if you want to change this slot to ${CARD_TYPE_LABELS[type] || "this type"}.`,
      );
      return;
    }
  ...
  const addNewCard = (cardType = editedCard, shouldSetActive = true) => {
    if (cards.length >= MAX_CUSTOMIZABLE_CARDS) {
      toast.warn(
        `You can customize up to ${MAX_CUSTOMIZABLE_CARDS} cards only.`,
      );
      return;
    }
  ...
  const compositeCardToBase64 = async (card) => {
    ...
    if (card.userPhoto) {
      ...
      if (isJoker) {
        ...
        const dw = img.width * ratio * zoom,
          dh = img.height * ratio * zoom;
  ...
  const goToFinalView = async ({
    ...
    boxImages: passedBoxImages = [],
  } = {}) => {
    const hasAllCards = CARD_FLOW.every((req) =>
    ...
    setspinloading(false);
    router.push(
      redirectToCheckout ? "/my-cart/checkout" : "/final/photoportrait",
    );
  };
  ...
  const Done = async () => {
    ...
    const hasJokerCardNow = cards.some(
  ...
  const handleAddJokerCard = () => {
    const jokerBase =
      product?.customizations?.custom_sets?.find(
        (item) =>
          item?.card_type === "Joker_Card" || item?.name === "Joker_Card",
      )?.image || null;
    ...
    setCards((prev) => {
      const existingJokerIndex = prev.findIndex(
        (card) => card?.editedCard === "Joker_Card",
      );
  ...
  const allCardsDone = CARD_FLOW.every((req) =>
    cards.some((item) => item.editedCard === req),
  );
      ...
      <div className="bg-[#f2f4f8]">
        {showJokerUpsell ? (
          <main
            ...
            style={{
              background:
                "linear-gradient(135deg, #3CA9FF 0%, #8CCEFF 35%, #D9EEFD 62%, #EBF6FF 82%, #F3F4F6 100%)",
            }}
          >
            <div
              className="pointer-events-none absolute -left-16 top-10 h-44 w-44 rounded-[28px] opacity-80"
              style={{
                background: "linear-gradient(140deg, #3CA9FF, #7DC8FF)",
              }}
            />
            <div
            ...
            />
            <div
              className="pointer-events-none absolute bottom-10 right-[10%] h-28 w-28 rounded-[20px] opacity-75"
              style={{
                background: "linear-gradient(145deg, #3CA9FF, #B3DEFF)",
              }}
            />
            <div
            ...
            >
              <div className="grid grid-cols-1 gap-5 md:grid-cols-[1fr_1.1fr] md:gap-7">
                <div className="order-2 space-y-4 md:order-1 md:space-y-5">
                  ...
                  <div
                    className="space-y-2.5 rounded-2xl p-3 sm:p-4"
                    style={{
                      background: "#F3F4F6",
                      border: "1px solid #D9EEFD",
                    }}
                  >
                    <button
                      ...
                      style={{
                        background:
                          "linear-gradient(90deg, #3CA9FF 0%, #6AC0FF 100%)",
                ...
                <div className="order-1 flex flex-col items-center md:order-2">
                  <div
                    ...
                    style={{
                      background:
                        "linear-gradient(135deg, #3CA9FF 0%, #6AC0FF 100%)",
          ...
          <>
                  ...
                  <div className="mx-auto w-full max-w-[980px]">
                    <div className="flex w-full items-start justify-between">
                      {stepFlow.map((step, index) => {
                        ...
                        const isActive = isBoxStep
                          ? activeStep === "box"
                          : activeType === step.type;
                        const isCompleted = isBoxStep
                          ? activeStep === "box"
                          : cards.some(
                              (card) => card?.editedCard === step.type,
                            ) && !isActive;
                        const isSelectable = isBoxStep
                          ? allCardsDone
                          : cards.some(
                              (card) => card?.editedCard === step.type,
                            );
                        return (
                          <Fragment key={step.type}>
                            ...
                            >
                              <div
                                className={`flex h-10 w-10 items-center justify-center rounded-full border text-lg transition-all md:h-11 md:w-11 md:text-xl ${isActive ? "border-[#3CA9FF] bg-[#3CA9FF] text-white shadow-lg shadow-indigo-300" : isCompleted ? "border-indigo-200 bg-[#B8E6FE] text-indigo-700" : "border-gray-300 bg-gray-100 text-gray-400"}`}
                              >
                                {isCompleted ? (
                              ...
                              <p
                                className={`text-xs font-semibold md:text-sm ${isActive ? "text-[#3CA9FF]" : "text-gray-500"}`}
                              >
                                {step.label}
                              </p>
                            </button>
            ...
            <main className="grid w-full grid-cols-1 items-start xl:grid-cols-[260px_minmax(0,1fr)_350px] xl:h-[calc(100dvh-148px)]">
              ...
              <section className="relative flex flex-col self-start items-center justify-center overflow-hidden px-3 pt-4 pb-2 md:px-6 md:pt-6 md:pb-4 xl:pt-6 xl:pb-4">
                <StepHeading
                  activeType={
                    activeStep === "box" ? "Box_Customization" : activeType
                  }
                      ...
                      <div className="relative z-10 flex w-full max-w-[980px] flex-col items-center mx-auto">
                        <div className="relative flex min-h-[420px] w-full items-center justify-center">
                          <PhotoPortraitBoxPreview
                            ref={boxPreviewRef}
                            boxImages={boxImages}
                            onImagePositionChange={updateBoxImagePosition}
                          />
                        </div>
                      </div>
                    </div>
