            ...
            <div className="flex flex-col h-full">
                ...
                <div className="flex-1 overflow-y-auto px-4 py-3 pb-28 space-y-4">
                    <PanelContent
                        ...
                        setcardfinder={setcardfinder}
                        backImages={backImages}
                        baseBack={baseBack}
                        setBaseBack={setBaseBack}
                        backDescription={backDescription}
                        setBackDescription={setBackDescription}
                        backHighlightsTitle={backHighlightsTitle}
                        setBackHighlightsTitle={setBackHighlightsTitle}
                        backHighlights={backHighlights}
                        addBackHighlight={addBackHighlight}
                        removeBackHighlight={removeBackHighlight}
                        updateBackHighlightText={updateBackHighlightText}
                        updateBackHighlightIcon={updateBackHighlightIcon}
                        activeBackHighlightPicker={activeBackHighlightPicker}
                        setActiveBackHighlightPicker={setActiveBackHighlightPicker}
                        backLegacyTagline={backLegacyTagline}
                        setBackLegacyTagline={setBackLegacyTagline}
                        ...
                        packageTitlelimite={packageTitlelimite}
                        cardtiltelimite={cardtiltelimite}
                        carddes={carddes}
                        setcarddes={setcarddes}
                        carddeslimite={carddeslimite}
                        name={name}
                        setname={setname}
                        namelimite={namelimite}
                        name2={name2}
                        setname2={setname2}
                        name2limite={name2limite}
                        name3={name3}
                        setname3={setname3}
                        ...
                        setlabelthree={setlabelthree}
                        acarddate={acarddate}
                        setacarddate={setacarddate}
                        acarddatelimite={acarddatelimite}
                        attrIconOne={attrIconOne}
                        setAttrIconOne={setAttrIconOne}
                        attrIconTwo={attrIconTwo}
                        setAttrIconTwo={setAttrIconTwo}
                        attrIconThree={attrIconThree}
                        setAttrIconThree={setAttrIconThree}
            ...
            <div className="w-full flex lg:hidden items-center justify-center">
                ...
                >
                    <div className="bg-sky-300 w-[100px] h-[10px] rounded-full flex items-center justify-center p-2">
                        <svg xmlns="http://www.w3.org/2000/svg" className={`text-white w-3 h-3 transition-transform ${!smallconOpen ? "rotate-180" : ""}`} viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round"><polyline points="6 9 12 15 18 9"/></svg>
                    </div>
                </div>
            </div>
...
function PanelContent({
    sidebarTab,
    filteredTemplates,
    baseFront, setBaseFront, setcardfinder,
    backImages, baseBack, setBaseBack,
    backDescription, setBackDescription,
    backHighlightsTitle, setBackHighlightsTitle,
    backHighlights, addBackHighlight, removeBackHighlight,
    updateBackHighlightText, updateBackHighlightIcon,
    ...
    acarddate, setacarddate, acarddatelimite,
                        ...
                        {filteredTemplates.map(([key, tpl]) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative overflow-hidden rounded-lg border transition-all duration-200 cursor-pointer ${isSelected ? "border-sky-500 ring-2 ring-sky-200 shadow-md" : "border-slate-200 hover:border-slate-300 hover:shadow-sm"}`}
                                    type="button"
                                    aria-label={`Select front base card ${key}`}
                                >
                                    <Image src={tpl.image} width={1000} height={1000} alt={`front-${key}`} className="w-full aspect-[3/4] object-contain bg-slate-100" />
                                    {isSelected && (
                        ...
                        {backImages?.map((img, idx) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative overflow-hidden rounded-xl border transition-all duration-200 cursor-pointer ${isSelected ? "border-sky-500 ring-2 ring-sky-200 shadow-md" : "border-slate-200 hover:border-slate-300 hover:shadow-sm"}`}
                                    type="button"
                                    aria-label={`Select back base card ${idx + 1}`}
                                >
                                    <Image src={img?.image} width={1000} height={1000} alt={`back-${idx}`} className="w-full aspect-[5/7] object-contain bg-slate-100" />
                                    {isSelected && (
                                        <div className="absolute top-1.5 right-1.5 rounded-full bg-white/95 p-1 shadow-sm">
                                            <BsCheckCircleFill className="text-sky-500 text-sm" />
                                        </div>
                    ...
                    <div>
                        ...
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            <button
                                ...
                                className={`h-9 rounded-xl border px-3 text-sm font-semibold transition-all duration-200 ${backHighlights.length >= 6 ? "cursor-not-allowed border-slate-200 bg-slate-100 text-slate-400" : "border-slate-300 bg-white text-slate-700 hover:border-slate-400"}`}
                            >
                                + Add
                            </button>
                        </div>
            ...
            {sidebarTab === "attributes" && (
                <div className="space-y-4">
                    ...
                    <div className="rounded-2xl border border-sky-100 bg-gradient-to-br from-sky-50 to-white p-5 shadow-sm">
                        <div className="mb-4">
                            <h3 className="text-lg font-semibold text-slate-900">Packaging Information</h3>
                        </div>
                        ...
                        <label className="mb-2 flex items-center justify-between text-sm font-medium text-slate-700">
                            ...
                            <CharactersCountComponent text={packageTitle} limit={packageTitlelimite} />
                        </label>
                        <input
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                <div className="relative flex items-center gap-3">
                                    <button type="button" onClick={() => setActiveIconPicker((prev) => (prev === "one" ? null : "one"))} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-slate-300 bg-white shadow-sm">
                                        {renderIconPreview(attrIconOne, "Attribute one icon")}
                                    </button>
                                    <input value={name} maxLength={namelimite} onChange={(e) => setname(e.target.value)} type="text" placeholder="Attribute One" className="h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-slate-700 outline-none transition-all duration-200 focus:border-sky-300 focus:ring-2 focus:ring-sky-100" />
                                    {activeIconPicker === "one" && (
                                        <div className="absolute top-14 left-0 z-30 w-[255px] rounded-xl border border-slate-200 bg-white p-3 shadow-xl">
                                            <div className="grid grid-cols-8 gap-2">
                                                {attributeIconOptions.map((icon) => (
                                                    <button key={`one-${icon}`} type="button" onClick={() => { setAttrIconOne(icon); setActiveIconPicker(null); }} className="flex h-8 w-8 items-center justify-center rounded-md p-1 hover:bg-slate-100">
                                ...
                                <div className="mt-3 mb-1 flex items-center justify-between text-sm text-slate-600">
                                    <span>Value</span><span className="font-medium text-slate-800">{labelone}</span>
                                </div>
                                <input min={1} max={100} value={labelone} onChange={(e) => setlabelone(e.target.value)} type="range" className="h-2 w-full cursor-pointer appearance-none rounded-full" style={getSliderTrackStyle(labelone)} />
                            </div>

                            {/* Attribute Two */}
                            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                <div className="relative flex items-center gap-3">
                                    <button type="button" onClick={() => setActiveIconPicker((prev) => (prev === "two" ? null : "two"))} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-slate-300 bg-white shadow-sm">
                                        {renderIconPreview(attrIconTwo, "Attribute two icon")}
                                    </button>
                                    <input value={name2} maxLength={name2limite} onChange={(e) => setname2(e.target.value)} type="text" placeholder="Attribute Two" className="h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-slate-700 outline-none transition-all duration-200 focus:border-sky-300 focus:ring-2 focus:ring-sky-100" />
                                    {activeIconPicker === "two" && (
                                        <div className="absolute top-14 left-0 z-30 w-[255px] rounded-xl border border-slate-200 bg-white p-3 shadow-xl">
                                            <div className="grid grid-cols-8 gap-2">
                                                {attributeIconOptions.map((icon) => (
                                                    <button key={`two-${icon}`} type="button" onClick={() => { setAttrIconTwo(icon); setActiveIconPicker(null); }} className="flex h-8 w-8 items-center justify-center rounded-md p-1 hover:bg-slate-100">
                                                        {renderIconPreview(icon, "Attribute icon option")}
                                                    </button>
                                                ))}
                                            </div>
                                ...
                                <div className="mt-3 mb-1 flex items-center justify-between text-sm text-slate-600">
                                    <span>Value</span><span className="font-medium text-slate-800">{labeltwo}</span>
                                </div>
                                <input min={1} max={100} value={labeltwo} onChange={(e) => setlabeltwo(e.target.value)} type="range" className="h-2 w-full cursor-pointer appearance-none rounded-full" style={getSliderTrackStyle(labeltwo)} />
                            </div>

                            {/* Attribute Three */}
                            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                <div className="relative flex items-center gap-3">
                                    <button type="button" onClick={() => setActiveIconPicker((prev) => (prev === "three" ? null : "three"))} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-slate-300 bg-white shadow-sm">
                                        {renderIconPreview(attrIconThree, "Attribute three icon")}
                                    </button>
                                    <input value={name3} maxLength={name3limite} onChange={(e) => setname3(e.target.value)} type="text" placeholder="Attribute Three" className="h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-slate-700 outline-none transition-all duration-200 focus:border-sky-300 focus:ring-2 focus:ring-sky-100" />
                                    {activeIconPicker === "three" && (
                                        <div className="absolute top-14 left-0 z-30 w-[255px] rounded-xl border border-slate-200 bg-white p-3 shadow-xl">
                                            <div className="grid grid-cols-8 gap-2">
                                                {attributeIconOptions.map((icon) => (
                                                    <button key={`three-${icon}`} type="button" onClick={() => { setAttrIconThree(icon); setActiveIconPicker(null); }} className="flex h-8 w-8 items-center justify-center rounded-md p-1 hover:bg-slate-100">
                                                        {renderIconPreview(icon, "Attribute icon option")}
                                                    </button>
                                                ))}
                                            </div>
                                ...
                                <div className="mt-3 mb-1 flex items-center justify-between text-sm text-slate-600">
                                    <span>Value</span><span className="font-medium text-slate-800">{labelthree}</span>
                                </div>
                                <input min={1} max={100} value={labelthree} onChange={(e) => setlabelthree(e.target.value)} type="range" className="h-2 w-full cursor-pointer appearance-none rounded-full" style={getSliderTrackStyle(labelthree)} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="border border-gray-200 p-4 md:p-5 mb-4 rounded-xl bg-white shadow-sm ring-1 ring-gray-100">
                        ...
                        <div className="w-full flex flex-col">
                            ...
                            <input value={acarddate} maxLength={acarddatelimite} onChange={(e) => setacarddate(e.target.value)} type="text" className="border border-slate-200 px-3 py-2 rounded-lg text-slate-700 outline-none transition-all duration-200 focus:border-sky-300 focus:ring-2 focus:ring-sky-100" />
                        </div>
                    </div>
                </div>
            )}
