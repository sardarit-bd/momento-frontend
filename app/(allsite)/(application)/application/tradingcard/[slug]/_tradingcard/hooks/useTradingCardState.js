...
export function useTradingCardState() {
  ...
  const packageConfig =
    PACKAGE_CONFIG[selectedPackage] ?? PACKAGE_CONFIG["single"];
      ...
      if (customizationStorageKey) {
        try {
          ...
          if (saved) {
            ...
            setAttrIconOne(
              saved?.content?.attrIconOne ??
                "/attribute-images/attribute_2.png",
            );
            setAttrIconTwo(
              saved?.content?.attrIconTwo ??
                "/attribute-images/attribute_3.png",
            );
            setAttrIconThree(
              saved?.content?.attrIconThree ??
                "/attribute-images/attribute_4.png",
            );
            setBackDate(saved?.content?.backDate ?? "");
            setBackDescription(saved?.content?.backDescription ?? "");
            setBackHighlightsTitle(
              saved?.content?.backHighlightsTitle ?? "Highlights",
            );
            setBackLegacyTagline(
              saved?.content?.backLegacyTagline ?? "A moment",
            );
            setBackLegacyText(
              saved?.content?.backLegacyText ??
                "This card celebrates a special person and a special time. May it remind you of all the great memories we've shared.",
            );
          ...
          for (const k of allSlotKeys) {
            ...
            if (
              slotData.selectedPackage &&
              selectedPackage &&
              slotData.selectedPackage !== selectedPackage
            )
              continue;
          ...
          if (validSlots.length > 0) {
            ...
            if (allDone) {
              ...
              setAttrIconOne(
                s.attrIconOne ?? "/attribute-images/attribute_2.png",
              );
              setAttrIconTwo(
                s.attrIconTwo ?? "/attribute-images/attribute_3.png",
              );
              setAttrIconThree(
  ...
  const captureCardSide = async (side) => {
    ...
    return captureNodeClean(previewCardNodeRef.current, (node) =>
  ...
  const resetCanvas = useCallback(
    (keepTemplate = false) => {
      if (!keepTemplate) {
        const firstTemplate =
          templateConfig?.image || (frontImages?.[0]?.image ?? null);
  ...
  const handleSaveSlot = async () => {
    ...
    if (
      editingSlotId === null &&
      savedSlotsRef.current.length >= packageConfig.designs
    ) {
      toast.warn(
        `Your ${packageConfig.name} package only allows ${packageConfig.designs} design(s).`,
      );
      return null;
    }
    ...
    try {
      ...
      if (editingSlotId !== null) {
        ...
        await idbPut(slotStorageKey(editingSlotId), {
        ...
        updatedSlots = savedSlotsRef.current.map((s) =>
          s.id === editingSlotId ? updatedSlot : s,
        );
        setEditingSlotId(null);
        ...
        const saved = await idbPut(slotStorageKey(slotId), {
          savedAt,
          selectedPackage,
          previewDataUrl,
          snapshot,
        ...
        if (!saved) {
          toast.error(
            "Storage full — could not save design. Please try removing an existing design.",
          );
          return null;
        }
  ...
  const handleEditSlot = (slot) => {
    ...
    setcardti(s.cardti ?? "Title");
    setcarddes(
      localStorage.getItem("persistent_carddes") ?? s.carddes ?? "Created For",
    );
    setPackageTitle(
  ...
  const backDateDisplay = backDate
    ? new Date(backDate).toLocaleDateString("en-US", {
        month: "2-digit",
        day: "2-digit",
        year: "numeric",
      })
