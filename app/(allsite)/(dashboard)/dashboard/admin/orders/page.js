...
function OrderTable({
  ...
  const openOrderModal = async (order, type) => {
    ...
    if (type === "receipt") {
      ...
      try {
        const res = await MakeGet(
          `api/tgc/receipts/${order.tgc_receipt_id}`,
          token,
        );
        ...
        if (receipt?.shipping_address_id) {
          const addrRes = await MakeGet(
            `api/tgc/addresses/${receipt.shipping_address_id}`,
            token,
          );
          receipt._shippingAddress =
            addrRes?.data?.data || addrRes?.data || null;
        }
    ...
    const needsDetailFetch =
      type === "pdf" ? !hasPdfData(order) : !hasPngData(order);
    if (!needsDetailFetch) {
      setModalLoading(false);
      return;
    }
  ...
  return (
    <div className="w-full bg-white">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left border-l border-b border-r border-gray-200 pb-25">
            ...
            {allorders?.map((order) => (
              <tr key={order.id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-4 py-3 text-center">
                  <button
                    onClick={() => {
                      openOrderModal(order, "receipt");
                    }}
                    className="text-blue-600 hover:underline text-sm mr-3 cursor-pointer"
                  >
                    Receipt
                  </button>
                </td>
      ...
      <div className="flex items-center justify-between mt-4 text-sm text-gray-700">
        <p>
          Showing page {pagination?.current_page || 1} of{" "}
...
const TableModal = ({ setismodalopen, modalinfo, modaltype, modalLoading }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center overflow-y-auto px-4 sm:px-6 md:px-10">
      ...
      <div
        ref={modalRef}
        className="relative bg-white border border-gray-200 shadow-2xl rounded-2xl w-full max-w-4xl max-h-[calc(100vh-180px)] sm:max-h-[85vh] md:max-h-[80vh] flex flex-col overflow-hidden mt-32 sm:mt-0"
      >
              ...
              <div className="flex flex-wrap gap-4">
                {tuckboxImages.map((url, index) => (
                  <a // ← was missing
                    ...
                    rel="noreferrer"
                    className="block bg-white rounded-xl border border-slate-200 p-2 shadow-sm hover:shadow-md transition w-50"
                  >
                    <Image
                      src={url}
                      alt={`tuckbox-${index + 1}`}
...
function ReceiptView({ data }) {
  const handleDownloadPDF = async () => {
    ...
    setStyle(11, false);
    pdf.text(
      "We really appreciate your business. Please tell your friends about us.",
      marginL,
      y,
    );
    newLine(28);
  ...
  const items = data?.packing_list?.[0]?.packing_list
    ? Object.entries(data.packing_list[0].packing_list).map(([name, qty]) => ({
        name,
        qty,
      }))
    : [];
