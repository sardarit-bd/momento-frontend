...
const Dashboardlayout = ({ children }) => {
  ...
  return (
    <div className="h-fit text-black">
      {
        <div
          className={`fixed flex lg:hidden z-90 text-balck items-center justify-center rounded-md  p-2 ml-4 -translate-y-18.75 mt-4 cursor-pointer `}
        >
          {isSideberOpen ? (
            <RxCross2
              className="text-gray-700 text-3xl transition duration-300 hover:rotate-180"
              onClick={() => {
                setisSideberOpen(false);
              }}
            />
          ) : (
            <FaBars
      ...
      <div className="">
        <div
          className={`fixed bg-white border-r border-gray-200 w-62.5 h-screen px-3 py-4 z-50 ${isSideberOpen ? "block lg:block" : "hidden lg:block"}`}
        >
          ...
          <button
            ...
            className="bg-sky-400 text-white w-[90%] text-center py-2 rounded-md absolute bottom-24 left-3 cursor-pointer flex items-center justify-center gap-2"
          >
            {isLoading && <SpinLoader />}
            Log out
          </button>
        </div>
        <div className="px-6 w-full h-fit pt-6 pl-6 lg:pl-67.5 min-h-screen pb-6">
