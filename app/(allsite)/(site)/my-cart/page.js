...
const MyCart = () => {
  ...
  const checkoutDisabled =
    isLoading || cart.length === 0 || pricingLoading || pricingError;
            ...
            {cart.length > 0 ? (
              <div className="divide-y divide-dashed divide-[#1B2420]/15">
                {cart.map((item, index) => {
                  ...
                  const unitPrice = pricedLine
                    ? Number(pricedLine.unit_price)
                    : null;
                  const lineTotal = pricedLine
                    ? Number(pricedLine.line_total)
                    : null;
                        ...
                        <div className="min-w-0 flex-1 pt-0.5 pr-10 sm:pr-0">
                          ...
                          >
                            {unitPrice !== null ? (
                              ...
                              <p className="text-[#B65C4D] text-sm">
                                Unavailable
                              </p>
                            )}
                          </div>
          ...
          <div className="col-span-1 lg:col-span-4 bg-white rounded-3xl shadow-sm border border-[#1B2420]/[0.06] p-5 sm:p-6 lg:sticky lg:top-6">
            ...
            <div className="border border-[#C9A227]/25 p-3.5 rounded-2xl mb-5 flex gap-3 items-start">
              ...
              <p className="text-sm text-[#1B2420]/70 leading-relaxed">
                <b className="text-[#1B2420]">Important:</b> after a successful
                order, we&apos;ll contact you shortly to confirm the details.
              </p>
            </div>
            ...
            {pricingLoading ? (
              <p className="text-sm text-[#1B2420]/40 py-4 animate-pulse">
                Calculating total…
              </p>
            ) : pricingError ? (
              <p className="text-sm text-[#B65C4D] py-4">
                Couldn&apos;t verify prices. Please refresh.
              </p>
            ) : (
              <div style={{ fontFamily: "var(--font-mono)" }}>
                ...
                <div className="py-4 flex justify-between items-baseline">
                  ...
                  <span className="text-xl font-semibold text-[#1B2420]">
                    ${total.toFixed(2)}
                  </span>
                </div>
              </div>
      ...
      {cart.length > 0 && (
        <div className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white/95 backdrop-blur border-t border-[#1B2420]/10 px-4 py-3 shadow-[0_-6px_20px_rgba(0,0,0,0.06)]">
          <div className="container mx-auto flex items-center justify-between gap-4">
            <div>
              <p className="text-[11px] uppercase tracking-wide text-[#1B2420]/45">
                Total
              </p>
              <p
