...
const Navigation = ({ isOpen, setisOpen }) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4 lg:gap-7 text-black mt-10 lg:mt-0 px-4 pr-6 lg:pr-0 lg:pl-0 w-full h-full lg:h-auto">
        {navItems.map((item, index) => (
          <Link
            key={index}
            onClick={() => setisOpen(false)}
            className={`font-semibold text-md text-nowrap py-3 px-2 rounded-md lg:px-6 lg:py-3 hover:bg-sky-100 w-full ${pathname === item.link && "bg-[#3CA9FF] text-white"}`}
            href={item.link}
          >
            {item.name}
          </Link>
        ...
        {loginUser?.token && (
          <button
            ...
            className="lg:hidden mt-auto mb-32 flex items-center justify-center gap-2 w-full px-6 py-3 rounded-full bg-linear-to-r from-sky-400 to-blue-500 text-white font-bold hover:from-sky-500 hover:to-blue-600 transition-all shadow-lg hover:shadow-xl hover:scale-105 transform"
          >
