...
export default function PDFViewers({ url, fulldata, title = "PDF Preview" }) {
      ...
      <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-5 py-4">
        ...
        {url ? (
          <div className="flex items-center gap-5">
            <span className="bg-sky-200  px-4 py-1 rounded-md font-bold">
              Customaizable Card
            </span>
            <a
              href={url}
          ...
          {!url ? (
            fulldata?.order_type === "Simple" ? (
              <div className="flex h-full items-center justify-center p-6 text-sm text-rose-700">
                <Image
                  src={fulldata?.items?.[0]?.product?.image}
                  width={400}
                  height={400}
                  alt="Simple Card"
                />
